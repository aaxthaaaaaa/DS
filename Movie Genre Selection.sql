...
INNER JOIN movie m 
ON rm.movie_id = m.id 
WHERE category = 'actress' AND country LIKE '%India%' AND languages='Hindi'
...
WITH top_3_genre AS (select genre,
...
year, 
title AS movie_name,
worlwide_gross_income, RANK() OVER(ORDER BY worlwide_gross_income DESC) AS movie_rank
FROM movie AS m INNER JOIN genre g
ON m.id=g.movie_id
...
SELECT d.name_id, NAME, d.movie_id, duration, r.avg_rating, total_votes, 
m.date_published, Lead(date_published,1) OVER(PARTITION BY d.name_id ORDER BY date_published,movie_id ) AS next_date_published 
